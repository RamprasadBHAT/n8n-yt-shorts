...
The F5-TTS invocation intentionally uses only the installed 1.1.22-compatible synthesis flags: `--ref_audio`, `--ref_text`, `--gen_text`, and `--output_file`. The module has no speaker configuration and never passes `--speaker` because the installed CLI rejects it.
